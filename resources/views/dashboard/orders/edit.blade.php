...
@section('container')
    ...
    <div class="col-lg-8">
        <form method="POST" action="/dashboard/orders/{{ $order->id }}" class="mb-5" enctype="multipart/form-data">
            @method('put')
            @csrf
            <div class="mb-3">
                ...
                <input type="text" class="form-control @error('jenis') is-invalid @enderror" id="jenis" name="jenis"
                    required autofocus value="{{ old('jenis', $order->jenis) }}">
                @error('jenis')
            ...
            <div class="mb-3">
                ...
                <select class="form-select" name="client_id">
                    @foreach ($clients as $client)
                        @if (old('client_id', $order->client_id) == $client->id)
            ...
            <div class="mb-3">
                ...
                <input type="date" class="form-control @error('tgl') is-invalid @enderror" id="tgl" name="tgl"
                    required autofocus value="{{ old('tgl', $order->tgl) }}">
                @error('tgl')
                    <div class="invalid-feedback">{{ $message }}</div>
                @enderror
            </div>
